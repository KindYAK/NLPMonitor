...
class DocumentSearchForm(forms.Form):
    ...
    authors = forms.ModelMultipleChoiceField(queryset=Author.objects.annotate(num_docs=Count('document')).filter(num_docs__gte=1000), label="Авторы", required=False)
    ...
    tags = forms.ModelMultipleChoiceField(queryset=Tag.objects.annotate(num_docs=Count('document')).filter(num_docs__gte=1000), label="Теги", required=False)
