function run_range_plot_management(csrf_token) {
    function relayout_dynamics(range_from, range_to, id_to_skip) {
    ...
    setInterval(function () {
        ...
        if (last_range_from && last_range_to && last_range_from !== range_from && last_range_to !== range_to) {
            relayout_dynamics(0, 1, null);
        }
    }, 1111);
    ...
    $('.dynamics-plot').on('plotly_relayout',
        function (e) {
            ...
            n_redraw_plot += 1;
            if (n_redraw_plot % 3 !== 0) {
                return;
            }
            relayout_dynamics(range_from, range_to, e.target.id);
        }
    );
}
